  ...
  return (
    <DebounceSelect
      ...
      fetchOptions={async (search: string) => {
        ...
        const all =  await fetch('https://www.spendow.com/solapi/causes')
        .then((res) => res.json())
        .then((data) => {
          const causes = data.Causes.map(a => ({label: a.Name,
            value: a.CryptoAccounts[0].Address}))
            const all = items.concat(causes);
            return all;
        }) 
        return all;
      }}
